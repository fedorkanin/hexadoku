...
class Solution {
  ...
  // the function also propagates constraints to other cells and deduce new
  ...
  // update constraints of the cell i,j by excluding possibility of
  // 'excludedValue' once there's one possibility left the function recurses
  // back into set()
  ...
  bool findValuesForEmptyCells() {
    ...
    // making backtracking efficient by pre-sorting empty cells by
    // numPossibilities
 ...
 public:
  void solveSudoku(vector<vector<char>>& board) {
    ...
    // As we do it - constraints are propagated and even additional values are
    // set as we go (in the case if it is possible to unambiguously deduce
    // them).
    for (int i = 0; i < 9; i++) {
      for (int j = 0; j < 9; j++) {
    ...
    if (!findValuesForEmptyCells()) return;  // sudoku is unsolvable
